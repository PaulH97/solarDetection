...
if os.path.exists("config_sen12.yaml"):
    with open('config_sen12.yaml') as f:
...
if preprocess_data:
    if model_sen12:
        ...
        tiles_list = filterScenes(sceneList, filterDate=False)
        [print(i) for i in tiles_list]
    
    else:
        tiles_list = ScenceFinderAOI(aoi_path, sentinel2_name, sentinel2_pl, sentinel2_pt, start_date, end_date, cloud_cover)
        [print(i) for i in tiles_list]
    ...
    for tile in tiles_list:
        ...
        if model_sen12:
            ...
            sen_path = glob(f"{tile[1]}/*.tif") + glob(f"{tile[0]}/*.tif") # B11 B12 B2 B3 B4 B5 B6 B7 B8 B8A
            sen_path.sort()
        else:
            tile_name = '_'.join(tile.split("_")[-2:])
            sen_path = glob(f"{tile}/*.tif") # B11 B12 B2 B3 B4 B5 B6 B7 B8 B8A
            sen_path.sort()
